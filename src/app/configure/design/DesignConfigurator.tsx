...
const DesignConfigurator: React.FC<DesignConfiguratorProps> = ({
  ...
  const [options, setOptions] = useState<{
    ...
    model: (typeof MODELS.option)[number];
  ...
  return (
    <div className="relative mt-20 grid grid-cols-1  lg:grid-cols-3 mb-20 pb-20 ">
      <div className="relative h-[37.5rem] overflow-hidden col-span-2 w-full max-w-4xl flex items-center justify-center rounded-lg border-2 border-dashed border-gray-300 p-12 text-center focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2">
        <div  ref={containerRef} className="relative w-60 bg-opacity-50 pointer-events-none aspect-[896/1831]">
          <AspectRatio
          ref={phoneCaseRef}
            ratio={896 / 1831}
          ...
          <div className="absolute z-40 inset-0 left-[3px] tops-px right-[3px] bottom-px rounded-[32px] shadow-[0_0_0_99999px_rgba(229,231,235,0.6)]" />
              ...
              <div className="flex flex-col gap-6">
                        ...
                        className={({ focus, checked }) =>
                          cn(
                            "relative -m-0.5 flex cursor-pointer items-center justify-center rounded-full p-0.5 active:ring-0 focus:ring-0 active:outline-none focus:outline-none border-2 border-transparent",
                            {
                              ...
                              "ring-2 ring-offset-2 ring-primary": focus,
                            }
                          )
                        }
                      >
                ...
                <div className="relative flex flex-col gap-3 w-full">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent>
                      {MODELS.option.map((model) => (
                        <DropdownMenuItem
                          key={model.label}
                          className={cn(
                            "flex test-sm gap-1 items-center p-1.5 cursor-default hover:bg-zic-100",
                            {
                              "bg-zinc-100":
                         ...
                         >
                          <span className="flex items-center ">
                           <span className="flex flex-col text-sm ">
                            <Label className="font-md text-gray-900">
                                {option.label}
                            </Label>
                            {option.description ? (
         ...
         <div className="w-full px-8 h-16 bg-white">
          ...
          <div className="w-full h-full flex justify-end items-center">
           ...
           <p className="font-medium whitespace-nowrap">{formatPrice(BASE_PRICE + options.finish.price + options.material.price)}</p>
            <Button size={"sm"} onClick={()=>saveConfiguration()}  className="w-full h-10 text-white">
              Continue
